...
REM // make sure we can write to the file bs2built.bin
REM // also make a backup to bs2built.prev.bin
IF NOT EXIST bs2built.bin goto LABLNOCOPY
IF EXIST bs2built.prev.bin del bs2built.prev.bin
IF EXIST bs2built.prev.bin goto LABLNOCOPY
move /Y bs2built.bin bs2built.prev.bin > NUL
IF EXIST s2built.bin goto LABLERROR3
REM IF EXIST bs2built.prev.bin copy /Y bs2built.prev.bin bs2built.bin
:LABLNOCOPY
...
"win32/s2p2bin" %s2p2bin_args% s2.p bs2built.bin s2.h

REM // fix some pointers and things that are impossible to fix from the assembler without un-splitting their data
IF EXIST bs2built.bin "win32/fixpointer" s2.h bs2built.bin   off_3A294 MapRUnc_Sonic $2D 0 4   word_728C_user Obj5F_MapUnc_7240 2 2 1  

REM REM // fix the rom header (checksum)
IF EXIST bs2built.bin "win32/fixheader" bs2built.bin
...
:LABLERROR3
echo Failed to build because write access to bs2built.bin was denied.
pause
